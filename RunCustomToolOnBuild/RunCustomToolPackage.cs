...
namespace RunCustomToolOnBuild
{
	...
	public sealed class RunCustomToolPackage : Package
	{
		...
		{
			LogActivity("============ Starting RunCustomTool post build action ============");
			LogActivity("Checking for any RunCustomTool attribute to run custom tool on template file..");
					...
					if (Scope == vsBuildScope.vsBuildScopeProject)
					{
						...
						if (currentProject != null && currentProject.ProjectItems != null)
						{
							foreach (ProjectItem projectItem in currentProject.ProjectItems)
								await CheckProjectItems(projectItem);
						}
						else
						{
			...
			LogActivity("============ Finished RunCustomTool post build action ============");
		}
		...
		private async System.Threading.Tasks.Task CheckProjectItems(ProjectItem projectItem)
		{
			if (WillRunCustomToolOnBuild(projectItem))
			{
				await RunCustomTool(projectItem);
				
			}
			if (projectItem.ProjectItems != null && projectItem.ProjectItems.Count > 0)
			{
				foreach (ProjectItem innerProjectItem in projectItem.ProjectItems)
					await CheckProjectItems(innerProjectItem);
			}
		}

		private async System.Threading.Tasks.Task RunCustomTool(ProjectItem projectItem)
		{
